...
train_max_acc = -np.inf

weights_path = './weights_debiasing/'

valid_steps_per_epoch = train_generator.n//train_generator.batch_size
...
for epoch in range(epochs):
    ...
    stp = 0
    for valid_step, (x0_batch_valid, x1_batch_valid, y_batch_valid) in enumerate(train_dataset):
        train_loss_ += test_step(x0_batch_valid, y_batch_valid, x1_batch_valid)
        if valid_step == (valid_steps_per_epoch-1):
            stp = valid_step
            break

    train_acc = valid_acc_metric.result()
    train_loss = valid_loss_metric.result()
    train_loss = train_loss/(stp+1)
    train_loss_ = train_loss_/(stp+1)

    train_acc_epoch.append(train_acc)
    train_loss_epoch.append(train_loss)
    train_custom_loss_epoch.append(train_loss_)
    print("Train acc: %.4f" % (float(train_acc),))
    print("Train custom loss: %.4f" % (float(train_loss_),))
